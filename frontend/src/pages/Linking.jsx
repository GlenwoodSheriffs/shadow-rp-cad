...
export default function Linking({user}) {
  ...
  async function submit(e){e.preventDefault();setError('');try{const linked=await api('/link/verify',{method:'POST',body:JSON.stringify({token,steamId})});setResult(linked);setTimeout(()=>location.reload(),1800)}catch(err){setError(err.message)}}
  return <div><div className="page-heading"><div><p className="eyebrow">GLENWOOD IDENTITY BRIDGE</p><h1>Account linking</h1><p>Connect this Discord account to your in-game {BRAND.city} identity.</p></div></div>
    <div className="link-grid"><section className="panel instructions"><h2><Gamepad2/> In-game steps</h2><ol><li>Join the {BRAND.city} Reforger server.</li><li>Your first join automatically shows a code at the top of the screen—no terminal is needed.</li><li>Copy the six-character code and enter it here within ten minutes.</li><li>If it expires, ask an administrator to reset your one-time link message.</li></ol><div className="linked-state"><span>Current Reforger UID</span><strong>{user.reforger_uid||'Not linked'}</strong></div></section>
